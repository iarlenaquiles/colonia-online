                  ...
                  <div class="widget-box">
                    ...
                    <p align="center"><?php echo '<img width="60" height="60" src="'.base_url($dadosColonia['foto']).'"/>';?></p>
                      <div class="widget-title">
                          <h4 style="text-align: center">Imposto Sindical</h4>
                      </div>
                  ...
                  <table class="table table-bordered">
                      <thead>
                          <tr>
                              ...
                              <th style="font-size: 1.2em; padding: 5px;">Data Pag</th>
                              <th style="font-size: 1.2em; padding: 5px;">Valor Pag</th>

                          </tr>
                      </thead>
                      <?php if($pagamentos > 0){ $valot_total;?>
                      <tbody>
                          <?php

                            foreach ($pagamentos as $a) {

                              echo '<tr>';
                              echo '<td>' . $a['numero_carteira'] . '</td>';
                              echo '<td>' . $a['nome_pessoa'] . '</td>';
                              echo '<td>' . $a['cpf_cnpj'] . '</td>';
                              echo '<td align="center">' . date('d/m/Y', strtotime($a['data_pagamento'])). '</td>';
                              echo '<td align="center">' . $a['valor_pagamento'].'</td>';

                              echo '</tr>';

                              $valot_total += (int) $a['valor_pagamento'];
                          }

                          ?>
                      </tbody>
                  </table>
                  Total: <?php echo count($pagamentos);?> <p align="right">Valor Total: <?php echo $valot_total; ?></p>
                  <? }else{
                    echo "Nenhum pagamento encontrado no ano selecionado";
                  } ?>
                  </div>
