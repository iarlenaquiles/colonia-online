                ...
                <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered"
                    ...
                    <script type="text/javascript">
                        function excluirRegistro(id) {
                            ...
                            $.ajax({
                                type: 'POST',
                                dataType: "json",
                                url: '<?php echo base_url("agendamento/excluir");?>/' + id,
                        ...
                        $(document).ready(function () {
                            $(".excluir").click(function (e) {
                                ...
                                confirmModal("Tem certeza que deseja excluir o Agendamento " + nomeRegistro + "?", 'excluirRegistro', null, codigoRegistro);
                            });
                    ...
                    if (0 < count($registros)) {
                        foreach ($registros as $registro) { ?>
                            <tr class="gradeU" >
                                <td><?php echo $registro['id_agendamento_inss'];?></td>
                                <td><?php echo $registro['nome_pessoa']; ?></td>
                                <td><?php echo $registro['tipo_beneficio']; ?></td>
                                <td><?php echo date('d/m/Y', strtotime($registro['data_agendamento'])); ?>,<?php echo $registro['horario'];?></td>
                                <td><?php echo date('d/m/Y H:i:s', strtotime($registro['data_cadastro'])); ?></td>
                                <td style="text-align: center;">
                                  <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'eAgendamento')){ ?>
                                    <a href="<?php echo base_url('agendamento/editar')."/".$registro['id_agendamento_inss'];?> " style="float:left;margin-left:4px;" title="Visualizar/Editar" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                                    <?php } ?>
                                    <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'dAgendamento')){ ?>
                                    <a href="" title="Excluir" class="btn btn-danger btn-sm excluir" style="float:left;margin-left:4px;" idRegistro="<?php echo $registro['id_agendamento_inss']; ?>" nomeRegistro="<?php echo $registro['id_agendamento_inss']; ?>"><i class="fa fa-trash-o "></i></a>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php
