<section class="wrapper site-min-height">
    ...
    <section class="panel">
        <header class="panel-heading">
            Lista de Atendimentos da <?php
              foreach($colonias as $colonia){
                if(!empty($this->session->userdata("colonia_id")) && $colonia['id_colonia'] == $this->session->userdata("colonia_id")){
                    ...
                    if (0 < count($registros)) {
                        foreach ($registros as $registro) { ?>
                            <tr class="gradeU" >
                                ...
                                <td><?php echo $registro['usuario_cadastro']; ?></td>
                                <td><?php echo date('d/m/Y H:i:s', strtotime($registro['data_cadastro'])); ?></td>
                        	<td style="text-align: center;"><a href="<?php echo base_url('pessoa/editar')."/".$registro['id_pessoa'];?> " style="float:left;margin-left:4px;" title="Visualizar/Editar" class="btn btn-primary btn-sm" target="_blank"><i class="fa fa-pencil"></i></a>
                          </td>
                            </tr>
                            <?php
                        }
